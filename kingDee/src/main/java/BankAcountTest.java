...
//            JSON parse = JSONUtil.parse(result);
    ...
    private static void getList() throws Exception {
        if (InvokeHelper.Login()) {
            ...
            String sContent = "{\n" +
                    "    \"FormId\": \"CN_BANKACNT\",\n" +
                    "    \"FieldKeys\": \"FNumber,FName,FForbidStatus,FBANKID,FBANKID.FName\",\n" +
                    "    \"FilterString\": \"\",\n" +
//                    "    \"FilterString\": \"FForbidStatus = 'A' and FDocumentStatus = 'C'  and FUseOrgId = '100098'\",\n" +
                    "    \"OrderString\": \"\",\n" +
                    "    \"TopRowCount\": 0,\n" +
                    "    \"StartRow\": 0,\n" +
                    "    \"Limit\": 0\n" +
...
//            deal(result);
    ...
    private static void deal(String bankAccount){
        List<List> bankAccountList = JSONUtil.toList(JSONUtil.parseArray(bankAccount), List.class);
        List<Map<String, Object>> mapList = new ArrayList<>();
        bankAccountList.forEach(val -> {
            Map<String, Object> map = new HashMap<>();
            map.put("bankAccount", Convert.toStr(val.get(0)));
            map.put("accountName", Convert.toStr(val.get(1)));
            map.put("status", Convert.toStr(val.get(2)));
            map.put("bankNumer", Convert.toStr(val.get(3)));
            map.put("bankName", Convert.toStr(val.get(4)));
            mapList.add(map);
        });
